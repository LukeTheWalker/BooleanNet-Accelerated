...
int main(int argc, char * argv[]){
    ...
    // CUDA usually picks device 0 by default, or we can use cudaSetDevice.
    int deviceCount;
    ...
    cudaMalloc(&d_zero_flags, data_size_bytes);
    cudaMalloc(&d_expr_values, data_size_bytes);

    // Memcpy --------------------------------------------

    cudaMemset(d_impl_len, 0, sizeof(uint64_t));
    cudaMemset(d_symm_impl_len, 0, sizeof(uint64_t));
    cudaMemcpy(d_zero_flags, zero_flags.data(), data_size_bytes, cudaMemcpyHostToDevice);
    cudaMemcpy(d_expr_values, expr_values.data(), data_size_bytes, cudaMemcpyHostToDevice);

    // // Launch kernel ------------------------------------------

    auto const beginT = std::chrono::high_resolution_clock::now();
    launch_cuda_kernel(d_expr_values, d_zero_flags, n_rows, n_cols, statThresh, pvalThresh, d_impl_len, d_implications, d_symm_impl_len, d_symm_implications);
    ...
    cudaMemcpy(&impl_len_val, d_impl_len, sizeof(uint64_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(&symm_impl_len_val, d_symm_impl_len, sizeof(uint64_t), cudaMemcpyDeviceToHost);
    ...
    cudaMemcpy(implications.data(), d_implications, impl_len_val * sizeof(impl), cudaMemcpyDeviceToHost);

    std::vector<symm_impl> symm_implications(symm_impl_len_val);
    cudaMemcpy(symm_implications.data(), d_symm_implications, symm_impl_len_val * sizeof(symm_impl), cudaMemcpyDeviceToHost);
